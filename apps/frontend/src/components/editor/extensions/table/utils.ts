...
export const findTable = (selection: Selection) =>
  findParentNode(
    (node) =>
      (node.type.spec.tableRole as TableRole) === "table" &&
...
const predicate = (node: Node) =>
  (node.type.spec.tableRole as TableRole) &&
  /cell/i.test(node.type.spec.tableRole as TableRole);
  ...
  (type: "row" | "column") => (index: number) => (tr: Transaction) => {
    ...
    if (table) {
      ...
      if (index >= 0 && index < (isRowSelection ? map.height : map.width)) {
        ...
        // @ts-expect-error - at is not defined on type number
